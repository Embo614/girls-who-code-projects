...
#Confusion Matrix for top 3 misidentified classes
classes = sorted(y_test.unique())
cm = confusion_matrix(y_test, y_pred, labels=classes)

#Find mistakes for each class
errors_per_class = {}
for i, class_id in enumerate(classes):
    total_samples = cm[i].sum()
    correct_predictions = cm[i, i]
    errors_per_class[class_id] = total_samples - correct_predictions

#Sort classes by number of errors
worst_classes = sorted(
    errors_per_class, key=errors_per_class.get, reverse=True
)[:3]

print(f'----------')
print('Top 3 most frequently misidentified classes:')
for class_id in worst_classes:
  print(f'Class {class_id}: {errors_per_class[class_id]} mistakes')
